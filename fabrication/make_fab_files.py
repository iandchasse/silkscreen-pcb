...
Outputs, in production/other_fabs/ (the gerber zip is shared and stays in production/):
  ...
  - placement_bottom_kicad.csv   KiCad's own position export, SMD only, for both fabs
  - nextpcb_centroid.zip         the same placement file zipped, because NextPCB's centroid slot rejects .csv
...
"""
import argparse, csv, os, re, shutil, subprocess, sys, tempfile, time, zipfile
...
def main():
    ...
    # NextPCB's centroid slot takes zip / rar / xlsx / xls only, not csv: zip it (fixed timestamp, so it is stable)
    zi = zipfile.ZipInfo("placement_bottom_kicad.csv", date_time=(2026, 1, 1, 0, 0, 0))
    zi.compress_type = zipfile.ZIP_DEFLATED
    with zipfile.ZipFile(OUT / "nextpcb_centroid.zip", "w") as z:
        z.writestr(zi, (OUT / "placement_bottom_kicad.csv").read_bytes())
    ...
    print(f"placement_bottom_kicad.csv: {len(smd)} SMD rows;  assembly_drawing_bottom.pdf {drawing}")
