...
(`nextpcb_bom.csv`, `nextpcb_centroid.csv`, `split/`). How to regenerate: see the root README.
...
| Problem | Cause | Fix |
|---|---|---|
| Error listing `J1 J5 J6 SW1-SW5 SW7-SW11` | BOM had the 13 through-hole parts, our centroid was SMD-only | `nextpcb_centroid.csv` now covers every fitted BOM part (162 rows) |
...
| Duplicate JLC part numbers (found in the same session) | `bom_JLC_upload_v4_optimized.csv` listed `C14663` and `C28323` on two lines each, so JLC left `C24 C31 C20` unmatched | Lines merged in v4. `bom_JLC_upload_v3.csv` still has the same duplicates |
...
- Total about **$400**: roughly the same as, and slightly under, JLCPCB's price with through-hole assembly included.
  The NextPCB route leaves about 75-80 hand-soldered joints per board (about 800 for 10) and the unresolved DFA and
...
`split/nextpcb_centroid_smd.csv` (149 parts, designators identical), and hand-solder the through-hole parts. Whether
