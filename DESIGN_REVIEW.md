...
> **Status, 2026-09-21.** This review describes the board as it stood on 2026-09-17/18. A later block-by-block pass — [docs/final-review-2026-09-19/](docs/final-review-2026-09-19/FINAL_REVIEW.md) — led to further changes that are **not** re-analysed here: `R83` + `C34` on the DW01A, PPTC `F2` on `J6` pin 12, the alternate RTC `U14` (DNP), mounting hole `H6`, the `USB_STAT` ladder rescaled ×10, `R57` 1 MΩ → 10 MΩ, `R15` 10 k → 1 MΩ, `C2` to a 25 V part and a wider VBUS trunk. Where this document and [HARDWARE.md](docs/HARDWARE.md) disagree on a value, HARDWARE.md is current. In particular §4's idle-drain arithmetic uses `R57` = 1 MΩ (≈4 µA through the gate divider); with 10 MΩ it is ≈0.4 µA.
...
|---|---|---|
| **Battery reverse-insertion — APPLIED: "Fix 4"** | With USB powering the charger, Q8 (gate hard to GND) is enhanced by the charger-driven P+ and Q3's body diode completes a path into a reversed cell. Behavioral simulation of the as-built nets shows the **full ≈0.25 A programmed charge current flowing continuously through the reversed cell** plus ≈2 mA through U5's CS ESD diode (U5's own supply clamps near −0.1 V through R16). | TP4056 **CE** pin gated from a cell-polarity detector **referenced to raw B−** (Q9 BSS138 + Q2 AO3401A + R79–R82, all existing BOM lines, ≈10 µA idle — corrected 2026-09-18, see §4). Simulated safe in every case, including flat-cell recovery, hot-swap and latched-connector insertion, and confirmed wired into the schematic/PCB (netlist-verified). **Bench-verify per §13 with a current-limited emulator** before relying on it. See §4. |
...
**Status (updated 2026-09-18):** Q4, R37, J7, the battery fix (§4, "Fix 4"), L1 (47 µH), R14 (2.2 Ω) and C9 (4.7 µF) are all applied in the source (netlist-confirmed); D2 is accepted; R6 stays 4.7 kΩ pending the thermal check. A 2026-09-18 pre-order confirmation pass (`docs/audit-2026-09-18/`) reviewed the whole board against this state: no blocking finding, three component-value questions (C9, L1/R14, Q1's hand-build sourcing) resolved — see §4/§7/§8 and the audit file for detail. **Controlled prototype:** the Toolkit set (Gerber ZIP, `positions.csv`, `bom.csv`, 162 CPL rows including Q2/Q9/R79–R82) was regenerated 2026-09-18 12:22 and verified geometrically identical to the saved PCB; the v3/v4 upload BOMs were updated for L1/R14/C9 the same day. A second 2026-09-18 checkpoint (`docs/audit-2026-09-18/CHECKPOINT_2.md`) re-reviewed the saved design after those changes: no blocker; open items are listed there and in §13. **Repeatable product:** still gated on the §13 first-article measurements, above all the battery-fault bench check with a current-limited emulator. This review does not demonstrate that existing boards will fail.
...
*Snapshot of the 2026-09-16 audit; on 2026-09-18 the design has 179 references / 129 nets (111 named), 162 fitted + 10 DNP + 7 bare-copper, and a 162-entry Toolkit CPL that matches the saved PCB.*
...
| Optional RTC | U13, C30 | VBAT-only topology supported; fitted in the standard build (optional), ±5 ppm, no backup/wake function §10 |
...
| Mounting | H1–H6 | No assembly parts required; enclosure/antenna interaction remains physical qualification §11 |
...
**Two distinct VBUS-overvoltage questions, disposed of separately 2026-09-18.** (1) *Sustained fault above 5 V:* `R2`/`R3` (§3.1, 5.1 kΩ Rd) are the board's only CC signaling — there is no PD controller to negotiate a contract, so a spec-compliant source has no mechanism to impose anything above the 5 V default; owner reports this pattern has held up on prior boards with a similar CC arrangement. `F1` (6 V max) and CR1 (pulse-rated, not continuous) remain under-margined *if* a genuinely non-compliant/malfunctioning source is used, which is accepted as a residual risk rather than pursued further — this is a narrower and more acceptable category than "any USB-PD charger." (2) *Hot-plug ringing at nominal 5 V:* a separate, faster phenomenon — cable inductance against USB_VBUS's ~11 µF of bare ceramic can ring for tens of µs above 5 V regardless of PD status, and a worst-case linear model puts the peak at 6–7.4 V, above TPS2116's 6 V abs-max; TI's own TP4056 sheet names this and suggests 1–1.5 Ω of series damping. Judged low-probability given real chargers' soft-start behavior and no reported field issue, and kept as a first-article scope check (§13) rather than a pre-emptive design change.
...
### Applied 2026-09-17: charge-enable gated by a B−-referenced cell-polarity detector ("Fix 4")

**Applied to the schematic and PCB (netlist-confirmed as of the 2026-09-17 21:11 board save).** Rather than re-engineering the power switch, the fix removes the *aggressor*: the TP4056 only sources into P+ while its CE pin is high, and with CE low the BAT pin is isolated (< 2 µA in disable/sleep mode; the internal pass PMOS body diode is reverse-biased for VCC > BAT — C16581 sheet). CE is therefore driven high only while a correctly oriented cell is present.

| Part (all existing BOM lines) | Connection |
|---|---|
| Q_det = `Q9` — BSS138 (C7420339) | gate ← **B+** via R_a; **source → raw B−**; drain → node X (`/DET_NODE`) |
| R_a = `R79` — 100 kΩ (C25803) | B+ → Q_det gate |
| R_b = `R80` — 1 MΩ (C22935) | Q_det gate → **B−** (no cell ⇒ Vgs = 0 ⇒ off) |
| R_c = `R81` — 1 MΩ | X → 3V3 |
| Q_pass = `Q2` — **AO3401A (C15127)** | source → 3V3; gate → X; drain → **U11 pin 8 (CE)** |
| R_d = `R82` — 1 MΩ | CE → GND (default: charger disabled) |
| — | **CE → USB_VBUS tie removed.** Q3, Q8, R27, R56, R57, U5, Q1 unchanged. |

*(Applied with `Q_pass` = AO3401A, not the AO3419 named when this table was first drafted — AO3401A was already the board's adopted P-channel swap for Q2/Q3/Q7/Q8, see §12; same SOT-23 pinout, no change to the analysis below.)*

Correct cell → Q_det on → X ≈ B− → Q_pass on → CE = 3V3 → charging. Reversed or absent → X = 3V3 → Q_pass off → CE low → charger off. CE only ever sees 3V3 or GND, never raw battery. Neither transistor needs a low-threshold part: Q_det only sinks the 1 MΩ pull-up (gate = B− + 0.91·V_cell, 2.18 V at the 2.4 V DW01A cut-off against a 1.5 V worst-case Vth — simulated at that corner), and Q_pass drives a logic pin at Vgs ≈ −3.3 V.

**Idle drain — corrected 2026-09-18: ≈10 µA total, not ≈3.4 µA.** The original estimate only counted R_a+R_b (`R79`/`R80`) across the raw cell: `V_cell / (R79+R80) ≈ 3.7 V / 1.1 MΩ ≈ 3.4 µA` — that part is right, and is on top of the existing R56/R57 divider's own ≈3.7 µA. It missed R_c/R_d (`R81`/`R82`) on the **3V3 side**: whenever a correct cell is present, `/DET_NODE` sits at ≈B− and `CE` sits at ≈3V3, so each 1 MΩ resistor drops a full ≈3.3 V — `I ≈ 3.3 V / 1 MΩ ≈ 3.3 µA` apiece, ≈6.6 µA combined, drawn continuously from the always-on 3V3 rail (i.e. through the LDO, not directly from the cell). Total Fix 4 contribution ≈3.4 + 6.6 ≈ **10 µA**. Folded into the sleep-current estimate in `docs/HARDWARE.md` §16.

**Not worth reducing further.** Raising `R79`–`R82` to cut this trades directly against the leakage margin discussed two paragraphs below (`Q9`/`Q2` off-state leakage is already comparable to the bias current at 1 MΩ; going higher makes the logic-level margin worse, not better) for a saving smaller than `U3`'s own ≈25 µA quiescent draw. If deep-sleep current is ever worth chasing seriously, `U3`'s `EN` tied to `IN` (§8, hardware §3.5) is the larger, more separable target.

**Known, accepted limitation — not treated as a defect.** Fix 4 can only enable the charger while a correctly oriented cell shows above roughly 1–1.8 V at `J5` (`Q_det` gate = 0.909·V(J5), Vth 0.5–1.6 V depending on build). A pack whose own protection IC has already latched into over-discharge lockout (≈0 V at its leads) or a bare cell drained past that point cannot be revived by the on-board charger, because there is nothing left for `Q_det` to sense — a real behavior change from the pre-Fix-4 board, where CE tied straight to VBUS and the charger always tried. Judged an unrealistic/insubstantial scenario for this board's use case and **not pursued further**; if a rescue path is ever wanted, the node to drive is `Q_det`'s drain (`/DET_NODE`), e.g. a two-pad manual-override jumper to GND.
...
**Vendor package identity, resolved 2026-09-18.** A review round flagged that Fortune Semiconductor's own "FS8205A" datasheet (every revision 1.0–1.7, 2009–2016) is TSSOP-8 only — Fortune's SOT-23-6 dual-FET part is named "FS8205," no A — and the hand-build BOM's manufacturer field (`fabrication/BOM_handbuild_digikey.csv`, `part_fields.csv`) names Fortune for Q1. Re-checked directly against Fortune's datasheets and against LCSC/DigiKey listings: **"FS8205A" genuinely exists as an orderable SOT-23-6 part, DigiKey-findable, just not from Fortune.** EVVOSEMI sells it in SOT-23-6 (DigiKey PN `26220994`, their own datasheet's Ordering Info gives "Equivalent Part Number: FS8205A," pinout matches Q1's footprint exactly), and TECH PUBLIC's SOT-23-6 FS8205A is the LCSC `C2830320` already used on the JLC build. **Fix: change the Manufacturer field to EVVOSEMI (or TECH PUBLIC) — the MPN "FS8205A" itself is correct, it was only the assumed manufacturer that was wrong.** (`part_fields.csv`, the schematic/PCB fields and `fabrication/BOM_handbuild_digikey.csv` all carry EVVOSEMI as of 2026-09-18.)
...
Expansion GPIO3/45/46 are boot straps. Attached accessories must respect reset levels; setting software pulls afterward cannot repair a wrong sampled strap. GPIO46 is an ordinary input/output pin on the ESP32-S3 (the "input-only" restriction belongs to GPIO34–39 of the original ESP32, and an earlier draft of this review carried it over by mistake); what matters is that it is a strapping pin with a weak internal pull-down that must read low at reset. GPIO39–42 overlap JTAG roles, so firmware must configure them for SCL, COLOR_SEL, TP_INT and PWM_LED. Do not assume a debug configuration is harmless to connected peripherals. [ESP32-S3 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32-s3_datasheet_en.pdf), [Espressif schematic checklist](https://docs.espressif.com/projects/esp-hardware-design-guidelines/en/latest/esp32s3/schematic-checklist.html).
...
The original, TF PUSH and GCT contact numbering agrees: **1 DAT2, 2 DAT3, 3 CMD, 4 VDD, 5 CLK, 6 GND, 7 DAT0, 8 DAT1**. Shield/detect arrangement does not provide an independent firmware card-detect signal here. **Q7 (SD VDD switch) has no gate slow-down:** IO10 drives the gate through R78 (1 kΩ) and R40 (100 kΩ) holds it off, so the AO3401A turns on in microseconds into C36+C37 (≈1.1 µF) plus the card's own capacitance — faster than the SD spec's suggested minimum VDD ramp (~0.1 ms) and able to dip 3V3 by a few hundred mV briefly. Optional fix if the first article shows card-init glitches: 47–100 nF from gate to 3V3 (τ ≈ 50–100 µs with R78). *(Added 2026-09-18.)*
...
R14 raises the source: `VGS = VGDR − ISW × R14`. With R14 = 2.2 Ω (3 Ω before 2026-09-18) and GDR at 3.3 V, illustrative 100/200/300 mA pulses leave **3.1/2.9/2.6 V VGS** (3.0/2.7/2.4 V with the former 3 Ω). The local SSD1677 sheet does not sufficiently specify loaded GDR drive/RESE trip behavior to prove this substitution solely by calculation. R15 = 10 kΩ also draws 0.33 mA at a 3.3 V gate level.

Until 2026-09-18, L1 = 22 µH, R14 = 3 Ω and R15 = 10 kΩ differed from the reference's 47 µH / 2.2 Ω / 1 MΩ; L1 and R14 now match it and only R15 (10 kΩ vs 1 MΩ, a gate pull-down chosen for a defined off state) still differs. The earlier differences were a **qualification requirement**, not proof the values were wrong. Earlier absolute claims that 22 µH guarantees particular peak currents or that 47 µH is categorically wrong were unsupported. R14's 0.1 W corresponds to **183 mA RMS** at full continuous rating; permissible pulse current depends on duty, pulse capability and derating.

**Reference values confirmed 2026-09-18.** The earlier "panel reference p11" citation is now corroborated by a primary source: the Solomon Systech SSD1677 datasheet (§13 "Application Circuit," Table 13-1, p.45) specifies L=47 µH (Sumida CDRH2D18/LDNP-470NC), R(sense)=2.2 Ω, matching a `GDR`/`RESE`/peak-current-mode boost topology consistent with this board's Q4/L1/R14 — RESE's internal comparator threshold is not published, but the topology confirms `I_peak ≈ V_th(internal) / R14`, so the earlier `(22/47)×(2.2/3)² ≈ 0.25×` reference-energy estimate is on the right physical model.

**L1: applied 2026-09-18, 22 µH → 47 µH.** The initial survey (same-footprint 3.0×3.0 mm candidates only) found nothing workable — the one same-footprint 47 µH shielded part is TDK `VLS3012CX-470M-1`, DigiKey-only with a ~12-week lead time, and every in-stock LCSC 47 µH candidate in that size needed a visibly larger, unshielded footprint. Once footprint was no longer treated as a constraint, several dual/LCSC-side candidates were surveyed (SHOU HAN `CYA1040-47UH` and CENKER `CKCS6045`/`CKCS5040` — excellent LCSC pricing, not DigiKey-carried at all; TDK `VLS5045EX-470M-H` — genuinely dual-sourced but didn't beat LCSC-side cost, the owner's stated priority; a Bourns `SRN5040-470M` pairing — **initially read as shielded from an automated DigiKey summary, later found to be semi-shielded per Bourns' own datasheet title, "SRN5040 Series - Semi-shielded Power Inductors" — not used**). Final part: **Laird `TYS5040470M-10`** (prime/DigiKey — confirmed direct-sold, active, 5.00×5.00×4.20 mm, shielded per Laird's own datasheet ("magnetic shielded structure"), Isat 1.1 A, DCR 272 mΩ max, rated 1 A, $0.41/$0.34/$0.28 @1/10/100, 24.2k stock) paired with **Sunltech `SLW5040S470MST` / LCSC `C206267`** (JLC build — 5.0×5.0×4.0 mm, shielded, Isat 1.3 A, DCR 650 mΩ, $0.046/pc @qty10 down to $0.024/pc @qty10.5k, 1.85k stock). Both parts are shielded and the same physical size — no shielding or footprint asymmetry between builds.

**This is a footprint change, not a drop-in swap** — `L1`'s land needs to grow from 3.0×3.0 mm to 5.0×5.0 mm, and the nearest neighbors (a 0603 cap at 3.08 mm, a 0603 resistor at 3.46 mm, `Q4`'s SOT-23 at 3.93 mm, a SOD-123 diode at 4.35 mm, all measured center-to-center from `L1`'s old position via pcbnew) will very likely need to shift to make room. **Footprint applied: `Inductor_SMD:L_APV_ANR5040`** (1.4 mm pads, 2.3 mm gap, 5.1 mm total span) — checked directly against Bourns' own published recommended land pattern for this case class (1.5 mm pads, 2.1 mm gap, 5.1 mm span): a close, comfortably conservative match. A footprint generated for a Cenker part was tried first and rejected — its 1.1 mm pad gap was a full millimeter tighter than Bourns' recommendation, a real solder-bridging risk during reflow. Schematic Value/MPN/Manufacturer/LCSC fields need to match the parts above.

**R14: applied 2026-09-18, 3 Ω → 2.2 Ω.** Now Yageo `RC0603FR-072R2L` (JLC code `C22939`, a Basic UNI-ROYAL equivalent, since 2026-09-20; `C112307` before that), same 0603 1% series as the prior value; the former JLC code `C22356394` was an HKR `RCA033RFLF` at $0.0016 and `C112307` is $0.0092, so the change costs under a cent per board and both are Extended parts (the Extended-part count does not change). With L1 now genuinely at 47 µH, this lands the pair exactly on the SSD1677 reference operating point instead of partway there. **Modeled safe against every component rating** (peak current rises 3/2.2 ≈ 1.36×, per-pulse energy 1.36² ≈ 1.86×, still far under the new L1's 1.1–1.3 A Isat and Q4's current rating) **but has not been bench-verified** — scope GDR/RESE and confirm VGH/VGL/VSH settle within SSD1677 spec (Table 11-1) on the first boards.
...
**Re-measured 2026-09-18 after the L1 move.** `EINK_SW` copper is **15.38 mm** with no vias (was 16.53 mm); the `C10`→`L1`→`Q4`→`R14` pad-centre loop area grew from ≈24 to ≈33 mm² (+36 %) because the 5×5 mm inductor is bigger, and `Q4`→`D5`→`C14` from ≈23 to ≈26 mm². `GDR` and `RESE` are unchanged by the move: **36.3 mm** and **36.6 mm** by the current measurement (the 52.22/42.74 mm quoted above came from an earlier branch-inclusive method and should not be compared directly), and they still run **≈18 mm side by side on one layer at a 0.15 mm gap** — a coupling item for the first-article scope check. `C10` (3V3 input cap for `L1`) is now ≈4.1 mm from `L1` (≈3.1 mm before) and `L1` still has only one GND via within 4 mm. No DRC clearance regressions came from the move.
...
**C9 output-cap sizing — applied 2026-09-18, 1 µF → 4.7 µF.** TI's COUT section (§8.2.2.3) recommends 1–4.7 µF and explains the trade-off as output ripple vs. loop stability, not dimming behavior; the reference design and every characterization curve use 1 µF nominal. The populated 1 µF/50 V/0805/X7R part is a nameplate value — MLCC DC-bias derating at the actual 15–24 V LED-rail bias is typically 25–40 % for this dielectric/case/voltage combination, putting its *effective* capacitance around 0.6–0.8 µF, below TI's own stated 1 µF minimum. C9 is now **4.7 µF/50 V/0805** — Samsung `CL21A475KBQNNNE` (prime) / Samwha `CS2012X5R475K500NRE`, LCSC `C513770` (the same part as C11/C13–C17, so no new line item), still inside TI's 1–4.7 µF window. A 4.7 µF X5R 0805 at 50 V is estimated to keep roughly 1.0–1.8 µF at 24 V and 1.5–2.5 µF at 15 V (vendor DC-bias curves were not retrievable — confirm with Samsung SimSurfing), so it meets the 1 µF minimum but only narrowly at the highest string voltage.

*Does this reintroduce dimming flicker?* No — TPS923610's only dimming mode chops the **internal 200 mV reference**, which is low-pass filtered *before* it reaches the GM error amplifier driving the current loop (§7.3.8): "although a PWM signal is used for brightness dimming, only the LED DC current is modulated." There is no PWM-frequency current edge at the LED string for COUT to smear, so the larger C9 should be neutral-to-beneficial for ADIM-based dimming. TI does not publish a COUT-vs-settling-time curve, so this is architectural reasoning from the datasheet's control-loop description, not a bench number.

*Effect on the colour-blend re-slew time above:* `t ≈ C9 × ΔV_f / I_LED` uses the same capacitor for a different purpose (string-to-string transition, not PWM dimming). With derating the effective value is roughly 1–2 µF, so a 1 V string difference at 13.3 mA settles in ≈100–150 µs (up to ≈350 µs at the full nameplate) versus ≈75 µs with 1 µF — almost certainly imperceptible for a colour-temperature blend, but blending is unqualified by board waveforms, so check it at the populated value. **Also check on the bench:** at battery hot-plug the L2→body-diode→C9 path charges the larger capacitor and can ring briefly to roughly 2.5 A (about 1.3 A with 1 µF) — confirm L2's (`VLS252012HBX-100M-1`) saturation margin or accept a brief saturation.

### Inductor, protection and layout

*(Historical: the 2026-09-14 matched order carried TDK VLS252010HBX-4R7M-1 / C88528. L2 is now 10 µH, TDK VLS252012HBX-100M-1 / C88532 in the schematic and the upload BOMs — §12.)* The 4.7 µH analysis below applies to the earlier value. TDK's HBX is 4.7 µH with 1.4 A inductance-drop and 1.09 A temperature-rise current ratings, reasonable for this low-power normal load. The small-load converter can operate discontinuously, so a simple average-current plus half-ripple estimate is **not a guaranteed peak**. Startup, minimum inductance and fault current must be checked; U10's maximum current limit exceeds the inductor's rating. [TDK characteristics](https://product.tdk.com/en/search/inductor/inductor/smd/info?part_no=VLS252010HBX-4R7M-1).
...
- Multiple keys form parallel combinations and may alias other keys. Define supported combinations; this is not an independently readable eight-bit keypad. Two chords fall inside single-key tolerance and cannot be separated by the ADC alone: ladder 1 `SW3`+`SW9` ≈ 1.11 V (worst case 1.09–1.14 V) vs `SW3` alone 1.16–1.21 V, a gap under the ESP32-S3 ADC's ±50 mV error; ladder 2 `SW4`+`SW7` ≈ 1.67 V vs `SW4` 1.77–1.84 V. Firmware should reject any reading that is not close to one of the six clean levels of its ladder. *(Added 2026-09-18.)*
- **SW10:** R62/R76 yield approximately 3.0 V pressed; GPIO18 supports RTC-domain wake. The analog ladders do not provide reliable per-key digital wake for every voltage — specifically only `SW2` (RIGHT) and `SW1` (DOWN(1)), at ≈33 mV, give a valid logic low on IO1/IO4 (idle is 3.3 V); `SW3` at 1.19 V (≈0.36·VDD) is in the undefined band and the higher steps read as logic high, so `ext0`/`ext1`/light-sleep GPIO wake cannot fire for LEFT, OK, BACK, UP(1), DOWN(2) or UP(2). *(Quantified 2026-09-18.)* Use the dedicated wake button or a deliberately implemented low-power ADC strategy.
...
- **Battery-path trace width — applied 2026-09-18.** `B+`, `B−` and `P+` were 0.20–0.25 mm through a 0603 zero-ohm jumper; the owner widened them to 0.4 mm (all `B+`/`B−` segments and 77 of 78 `P+` segments; one short `P+` stub stays 0.25 mm) and changed the series jumper `R27` from 0603 to **0805** (`C17477` UNI-ROYAL 0805W8F0000T5E, Basic; prime part Yageo RC0805JR-070RL). That roughly doubles the copper cross-section and the jumper's current capability, so the earlier fault-heating concern under the DW01A trip window is largely closed; `R27` remains a finite-rating part — check drop and temperature in the first-article load test.
...
- **Mounting:** H1–H6 pads connect to GND. Metal standoffs/housing can therefore make electrical chassis connections; include them in final-stack RF/EMC and clearance checks.
- **Models are illustrative:** MJTP models versus ordered TS365ZJ, and the custom SD socket need drawing-based fit checks. A plausible 3D render is not tolerance approval. (The original "Bourns-named L1 footprint versus TDK" note is moot — L1's footprint changed entirely 2026-09-18, see §7/§12.)
...
|---|---|
| Battery protection language | Unconditional bare-cell/reverse-insertion claims removed; §4 "Fix 4" (CE gated by a B−-referenced detector) is applied and netlist-confirmed — remaining step is the §13 bench verification. |
...
| L2 | **Decided 2026-09-17: 4.7 µH → 10 µH, TDK VLS252012HBX-100M-1 `C88532`** (same 2520 land, 1.2 mm). The TPS923610 runs forced-continuous at 1.1 MHz at every load and TI characterizes it only with 10 µH / 1 µF; with 4.7 µH the 0.6 A p-p ripple dominates losses. Model with TDK's measured Rac: ≈6–7 mA less battery current at every brightness (−10 % at full, −25…35 % dimmed), a third less output ripple, no flicker either way (analog dimming). Details and bench check in `fabrication/BOM.md`. **Applied — the schematic reads 10u / VLS252012HBX-100M-1 / C88532.** Fallback if 4.7 µH is kept: HBX-4R7M-1 `C88528` (as built on both earlier orders), not HBU. |
| L1 | **Applied 2026-09-18: 22 µH → 47 µH, Laird `TYS5040470M-10`** (prime/DigiKey) / **Sunltech `SLW5040S470MST` `C206267`** (JLC), both 5.0×5.0×4.0 mm, both shielded. Matches the SSD1677 reference design's inductor value (§7). **Footprint change, not a drop-in** — land grows from 3.0×3.0 mm; `Inductor_SMD:L_APV_ANR5040` applied (pad gap checked against Bourns' published land pattern, a close match). The neighbours (C10, R63, Q4, D5) were moved to clear the new footprint and the EINK_SW/GDR/RESE lengths were re-measured on 2026-09-18 (§7); schematic and PCB fields match. |
| R14 | **Applied 2026-09-18: 3 Ω → 2.2 Ω, Yageo `RC0603FR-072R2L`** (JLC code `C22939` since 2026-09-20; was `C112307`) — same 0603 1% series/price as the prior part. Matches the SSD1677 reference design's sense resistor now that L1 (above) is also at the reference value. Modeled safe against every component rating; **not bench-verified** — see §7. |
| CR1 | **Decided 2026-09-17: SMF6.5CA in SOD-123FL** (prime: Littelfuse SMF6.5CA; JLC: hongjiacheng `C19077501`, Preferred, 1.75 M stock). The SMFxx ladder is standardized, so official and clone parts share the spec: 6.5 V standoff, V_BR 7.22–7.98 V, ≤11.2 V clamp @ 17.9 A (10/1000 µs), bidirectional — 1 V clear of USB-C's 5.5 V maximum, unlike "05"-named parts whose clones are 5.0 V / V_BR ≥ 6 V. Closes the §3 VBUS standoff finding. **Applied — CR1's footprint is `Diode_SMD:D_SMF`** (fits in place). CR2/CR3 (3V3, P+ ≤ 4.2 V) stay on the SOD-323 5.0 V part. |
| U13 / SW6 | **Standard-build policy (2026-09-17):** U13 (DS3231MZ+TRL, `C107410` — the reel listing; the earlier cut-tape `C722467` is superseded) is populated; SW6 (APEM MJTP1243 boot button) is DNP. The source DNP set is exactly TP3/TP4/TP5, R43/R45/R58/R66/R72/R74 and SW6 — ten references (eleven since 2026-09-21, when the alternate RTC `U14` was added as DNP), flagged DNP in both schematic and board so the Toolkit CPL agrees. Upload BOM: `production/jlc_bom.csv` (named `bom_JLC_upload_v4_optimized.csv` until 2026-09-21; updated 2026-09-18 for L1/R14/C9; the brand-conservative v3 file has been deleted) — see `fabrication/BOM.md` for the swap rationale and cost model. |
...
| Capacitor descriptions | C22 is near the panel, not MCU-local or RTC decoupling; RTC decoupler is C30. **C9 = 4.7 µF/50 V (applied 2026-09-18; was 1 µF)** — the 1 µF part derated below TI's recommended COUT minimum at the 15–24 V LED rail bias; the trade-off with the colour-blend re-slew time is in §8. |
...
| RTC | ±5 ppm, fitted in the standard build, documented VBAT-only operation; no independent backup or interrupt wake. |
...
| Assembly counts | 179 references = **162 fitted + 10 DNP + 7 bare-copper** (H1–H5, TP1, TP2) on 2026-09-18 — **as of 2026-09-21 it is 184 = 165 fitted + 11 DNP + 8 bare-copper** (H1–H6, TP1, TP2; `U14`, `F2`, `C34`, `R83` and `H6` were added); the 2026-09-16 audit counted 173 = 155 + 8 + 10, and older “163 placed” sourcing totals are historical. |
...
12. **Checks added by the 2026-09-18 changes and checkpoint** (L1 47 µH, R14 2.2 Ω, C9 4.7 µF). (a) With the panel powered, scope `GDR`, `RESE` (across R14), `EINK_SW` and the 3V3 ramp during power-up and a full refresh: peak inductor current must stay under 1.1 A (the Laird part's Isat; 1.3 A Sunltech), `VGH`/`VGL`/`VSH` must settle within SSD1677 spec (Table 11-1), and HV-Ready detection must still complete. (b) Compare refresh time and rail droop against the earlier 22 µH/3 Ω boards if any are at hand. (c) Frontlight: confirm ripple/stability at maximum brightness with the larger C9, blend re-slew time if blending is used, and L2's behavior at battery hot-plug. (d) `GDR`/`RESE` run ≈18 mm side by side at a 0.15 mm gap — check for coupling on both traces. (e) Hot-plug a short, thick USB-A-to-C cable into a stiff 5 V source and scope `USB_VBUS` at the input capacitors (peak vs the TPS2116's 6 V absolute maximum). (f) Confirm DS3231M I²C reads/writes with VCC grounded and VBAT on 3V3 (the DC table's VIH is specified against VCC).
